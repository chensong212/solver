 ...
 *  Purpose: Specify the main loop for the simulation
 *
 */
...
double getdt(int step)
{
	...
	else
	{
		...
		for(j=0; j<nc; j++)
		{
			u = sqrt(U.q[j][1]*U.q[j][1] + U.q[j][2]*U.q[j][2])
			  + sqrt(U.gam[j]*U.pre[j]/U.q[j][0]);
			if(u > um) um = u;
		}
		dt = CFL*dx/um;
	}
